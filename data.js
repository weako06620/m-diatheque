...
const INITIAL_BOOKS = [
  ...
  },
  {
    ...
    pages: 508,
    featured: false,
    description: "Ferdinand Bardamu parcourt le monde — la Grande Guerre, l'Afrique coloniale, l'Amérique — dans ce roman sombre et révolutionnaire qui a bouleversé la littérature française."
  },
  {
    id: 20,
...
/* ══════════════════════════════════════════════════════════════════════
   ...
   Les dates sont relatives à aujourd'hui pour que la démo soit toujours pertinente.
═══════════════════════════════════════════════════════════════════════ */

/**
 * Retourne une date ISO (YYYY-MM-DD) décalée de `offsetDays` jours
 * par rapport à aujourd'hui.
 * @param {number} offsetDays - Nombre de jours (négatif = passé, positif = futur)
 * @returns {string}
 */
...
const INITIAL_LOANS = [
  ...
  { id: 2,  bookId: 5,  borrower: 'Jean-Pierre Martin',  borrowedDate: relativeDate(-35), dueDate: relativeDate(-5),  returned: false }, // En retard
  ...
  { id: 7,  bookId: 7,  borrower: 'Nathalie Morel',      borrowedDate: relativeDate(-15), dueDate: relativeDate(-3),  returned: false }, // En retard
  { id: 8,  bookId: 16, borrower: 'Lucas Durand',        borrowedDate: relativeDate(-25), dueDate: relativeDate(5),   returned: false },
  { id: 9,  bookId: 1,  borrower: 'Isabelle Bernard',    borrowedDate: relativeDate(-60), dueDate: relativeDate(-30), returned: true  }, // Retourné
  { id: 10, bookId: 6,  borrower: 'Thomas Petit',        borrowedDate: relativeDate(-45), dueDate: relativeDate(-15), returned: true  }, // Retourné
];
...
const DB = {
  ...
  /**
   * Initialise le localStorage avec les données de démo
  ...
  /** Remet la base de données à l'état initial de démo. */
  reset() {
    localStorage.setItem(DB_KEYS.BOOKS, JSON.stringify(INITIAL_BOOKS));
    localStorage.setItem(DB_KEYS.LOANS, JSON.stringify(INITIAL_LOANS));
  },
  ...
  /** @returns {Array} Tous les livres */
  getBooks() {
    return JSON.parse(localStorage.getItem(DB_KEYS.BOOKS)) || [];
  },

  /** @returns {Object|null} Un livre par son id */
  getBook(id) {
    return this.getBooks().find(b => b.id === id) || null;
  },

  /**
   * Sauvegarde un livre (mise à jour si existant, ajout sinon).
   * @param {Object} book
   */
  saveBook(book) {
  ...
  /** @returns {Array} Tous les emprunts (actifs + retournés) */
  getLoans() {
    return JSON.parse(localStorage.getItem(DB_KEYS.LOANS)) || [];
  },

  /** @returns {Array} Emprunts actifs uniquement */
  getActiveLoans() {
    return this.getLoans().filter(l => !l.returned);
  },

  /**
   * Retourne l'emprunt actif pour un livre donné.
   * @returns {Object|null}
   */
   ...
   * @param {Object} loan - { bookId, borrower, borrowedDate, dueDate }
   */
  addLoan(loan) {
  ...
  /** @returns {boolean} Vrai si le livre est disponible (non emprunté) */
  ...
  getStats() {
    ...
    // Top 5 livres les plus empruntés (historique complet)
    const loanCounts = {};
    loans.forEach(l => {
...
const AUTH = {
  /** Mot de passe de démo — à remplacer par un vrai hash en production */
  _password: 'cipieres',
  _isAdmin: false,

  /**
   * Vérifie le mot de passe et active la session admin.
...
/**
 * Couleurs de gradient par genre.
...
/**
 * Retourne un gradient CSS pour un genre donné.
 * @param {string} genre
 * @returns {string}
 */
...
/**
 * Retourne le nombre de jours écoulés depuis une date.
...
/**
 * Retourne le nombre de jours restants jusqu'à une date (négatif si dépassée).
 * @param {string} dateStr
 * @returns {number}
 */
...
/**
 * Détermine la liste unique de genres dans le catalogue.
 * @returns {string[]}
 */
